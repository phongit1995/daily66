...
router.post("/tranfer-account",async(req,res)=>{
    ...
    const cookieCloudflare = await getCookieCloudflare(proxy);
    const resultTranfer =  await transferAccount(cookieCloudflare,USER_ARGENT,userSend.cookie,usernamereceive,amount,reason)
...
router.post("/find-history-agent-receive",async(req,res)=>{
    try {
        ...
        const cookieCloudflare = await getCookieCloudflare(proxy);
        const timeNow = moment().format("MM/DD/YYYY")
        const resultHistory = await findHistoryAgentReceive(cookieCloudflare,cookieCloudflare,USER_ARGENT,userSend.cookie,timeNow,timeNow,proxy);
